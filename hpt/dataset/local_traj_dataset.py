...
class LocalTrajDataset:
    ...
    def create_replaybuffer_from_env(self, env_rollout_fn):
        ...
        if self.task_description is not None:
            fixed_language_embedding = utils.get_t5_embeddings(self.task_description, per_token=True, device="cuda")
        else:
            fixed_language_embedding = None
        ...
        for traj_idx, episode in enumerate(dataset_iter):
            ...
            try:
                dataset_steps = []
                for step in tqdm.tqdm(episode["steps"], desc=f"Processing episode {traj_idx}", leave=False):
                    dataset_step = process_dataset_step(
                        ...
                        fix_language_embedding=fixed_language_embedding
                    )
